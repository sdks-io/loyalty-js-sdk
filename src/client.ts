...
export class Client implements ClientInterface {
  ...
  public clientAuthorizationTokenManager?: ClientAuthorizationTokenManager;
  ...
  constructor(config?: Partial<Configuration>) {
    ...
    this._userAgent = updateUserAgent(
      'TypeScript-SDK/0.0.6 (OS: {os-info}, Engine: {engine}/{engine-version})'
    );
    this._requestBuilderFactory = createRequestHandlerFactory(
      (server) => getBaseUri(server, this._config),
      createAuthProviderFromConfig(
        this._config,
        () => this.clientAuthorizationTokenManager
      ),
    ...
    if (this._config.clientAuthorizationTokenCredentials) {
      this.clientAuthorizationTokenManager = new ClientAuthorizationTokenManager(
        this._config.clientAuthorizationTokenCredentials,
        this
      );
    }
  }
...
function getBaseUri(server: Server = 'Shell', config: Configuration): string {
  if (config.environment === Environment.Sit) {
    ...
    if (server === 'Shell1') {
      return 'https://api-test.shell.com/loyalty/v1';
    }
    if (server === 'Shell2') {
      return 'https://api-test.shell.com/loyalty';
    }
  }
  if (config.environment === Environment.Production) {
    ...
    if (server === 'Shell') {
      return 'https://id.consumer.shell.com';
    }
    if (server === 'Shell1') {
      return 'https://api.shell.com/loyalty/v1';
    }
    if (server === 'Shell2') {
      return 'https://api.shell.com/loyalty';
    }
  }
  if (config.environment === Environment.Uat) {
    if (server === 'OAuth Server') {
      return 'https://api.shell.com';
    }
    if (server === 'Shell') {
      return 'https://test.id.consumer.shell.com';
    }
    if (server === 'Shell1') {
      return 'https://api-uat.shell.com/loyalty/v1';
    }
    if (server === 'Shell2') {
      return 'https://api-uat.shell.com/loyalty';
    }
  }
...
function withAuthenticationByDefault(rb: SdkRequestBuilder) {
  rb.authenticate([{ clientAuthorizationToken: true }]);
}
